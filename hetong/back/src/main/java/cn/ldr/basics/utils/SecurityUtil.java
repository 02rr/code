package cn.ldr.basics.utils;

import cn.ldr.basics.exception.ldrException;
...
import cn.ldr.data.vo.PermissionDTO;
import cn.ldr.data.vo.RoleDTO;
...
public class SecurityUtil {
    ...
    public String getToken(String username, Boolean saveLogin){
        if(ldrNullUtils.isNull(username)){
            throw new ldrException("username不能为空");
        }
        boolean saved = false;
        ...
        if(tokenProperties.getSaveRoleFlag()){
            for(PermissionDTO p : selectUser.getPermissions()){
                if(!ldrNullUtils.isNull(p.getTitle()) && !ldrNullUtils.isNull(p.getPath())) {
        ...
        if(tokenProperties.getSsoFlag()) {
            String oldToken = redisTemplate.get(ldrLoginProperties.USER_TOKEN_PRE + selectUser.getUsername());
        ...
        // 保存至Redis备查
        if(saved){
            redisTemplate.set(ldrLoginProperties.USER_TOKEN_PRE + selectUser.getUsername(), ansUserToken, tokenProperties.getUserSaveLoginTokenDays(), TimeUnit.DAYS);
            redisTemplate.set(ldrLoginProperties.HTTP_TOKEN_PRE + ansUserToken, JSON.toJSONString(tokenUser), tokenProperties.getUserSaveLoginTokenDays(), TimeUnit.DAYS);
        }else{
            redisTemplate.set(ldrLoginProperties.USER_TOKEN_PRE + selectUser.getUsername(), ansUserToken, tokenProperties.getUserTokenInvalidDays(), TimeUnit.MINUTES);
            redisTemplate.set(ldrLoginProperties.HTTP_TOKEN_PRE + ansUserToken, JSON.toJSONString(tokenUser), tokenProperties.getUserTokenInvalidDays(), TimeUnit.MINUTES);
        }
        return ansUserToken;
    }
